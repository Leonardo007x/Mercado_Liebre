...
**Pantallazos:** *(se incluyen después)*
...
Fallo numero 3 de 3
Circuito abierto
[mascotas/backend] Espera controlada (5s): nuevo intento de conexión…
[mascotas/backend] Fallo en intento de recuperación: ...
...
Sí. Cada servicio tiene su propia variable `circuito_abierto`. Si el backend falla 3 veces, solo se abre el circuito del backend. El circuito de usuarios permanece cerrado y sigue funcionando con normalidad.
...
```
# Al caer el servicio de usuarios:
Fallo numero 1 de 3
Fallo numero 2 de 3
Fallo numero 3 de 3
Circuito abierto

# Mientras backend sigue normal:
# GET /mascotas → responde con datos normalmente
# GET /usuarios → 503, circuito abierto
# GET /resumen  → responde parcialmente con datos de mascotas
```
...
Después de que el circuito lleva un tiempo abierto, se permite pasar **una sola petición de prueba**. En nuestro sistema eso ocurre cada vez que llega una petición mientras el circuito está abierto: se espera 5 segundos (`RECOVERY_WAIT_SECONDS = 5`) y se hace el intento.
...
Si el intento de prueba en estado half-open **falla**, el circuito **vuelve a abrirse** (o permanece abierto). El sistema imprime en logs:
...
### Flujo completo del circuit breaker
...
¿Circuito abierto?
  ...
  SÍ          NO
   │           │
   ▼           ▼
Esperar 5s   Intentar conexión
Intento de   ┌──────────────────┐
recuperación │ ¿Respondió bien? │
   │         └──────────────────┘
   ▼              │          │
¿Respondió?      SÍ         NO
 SÍ → OK       Reset      fallo++
 NO → 503    contador    ¿>=3 fallos?
  (abierto)   Cerrar      SÍ → Abrir
              circuito       circuito
```

**Pantallazos:** *(se incluyen después)*

### Logs de la fase de implementación

```
# Servicio caído, primeros intentos:
Fallo numero 1 de 3
Fallo numero 2 de 3
Fallo numero 3 de 3
Circuito abierto

# Primera petición con circuito abierto:
[mascotas/backend] Espera controlada (5s): nuevo intento de conexión…
[mascotas/backend] Fallo en intento de recuperación: HTTPConnectionPool(host='backend', port=5000): Max retries exceeded
[mascotas/backend] Recuperación fallida → circuito permanece abierto.

# Servicio vuelve a estar disponible (docker-compose start backend):
[mascotas/backend] Espera controlada (5s): nuevo intento de conexión…
[mascotas/backend] Recuperación OK → circuito cerrado.
```
...
```bash
docker-compose stop backend
curl http://localhost:5000/mascotas   # intento 1
curl http://localhost:5000/mascotas   # intento 2
curl http://localhost:5000/mascotas   # intento 3
```
...
```bash
curl http://localhost:5000/mascotas   # llega con circuito ya abierto
```
...
- **MySQL no espera**: cuando todos los servicios se levantan juntos con `docker-compose up`, el backend a veces intenta conectarse a la base de datos antes de que MySQL esté lista para aceptar conexiones. Esto genera errores al inicio que hay que esperar a que se resuelvan solos.
